...
print('-'*40+'KNeighborsRegression'+'-'*40)

neighbors_settings = range(1,300)
n_neighbors = 10
...
for neighbor in neighbors_settings:
  classifier=LogisticRegression(max_iter  = neighbor)
  classifier.fit(X_train,y_train)
  
  train_accuracy.append(np.mean(np.abs(classifier.predict(X_train)-y_train)))
  test_accuracy.append(np.mean(np.abs(classifier.predict(X_test)-y_test)))
  print(classifier.predict(X_test))
#找出最大的test_accuracy 的索引
max_test_accuracy = np.argmin(test_accuracy)
